...
def predict():
  class TwitterClient(object): 
    def __init__(self): 
        ...
        try: 
            ...
            auth = OAuthHandler('xxxxxxxxxxxxxxxxx','xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx') 
            # set access token and secret 
            auth.set_access_token('xxxxxxxxxxxxxx','xxxxxxxxxxxxxxxxx') 
  ...
  x = d.iloc[:,-2].values
  tv = TfidfVectorizer(max_df=0.90, min_df=2, stop_words='english',ngram_range=(1, 2))
  ...
  if request.method == 'POST':
    ...
    return render_template('result1.html',outputs = output,NU=Neucount,N=Negcount,P=Poscount,happy=h,sad=s,angry=a,loved=l,powerless=pl,surprise=su,fearless=fl,cheated=c,attracted=at,singledout=so,anxious=ax)
